 ...
 */
public class ExpChainingHashTable<K, V> extends DeletelessDictionary<K, V> {
    ...
    public ExpChainingHashTable(Supplier<Dictionary<K, V>> newChain) {
    ...
    private class ChainingHashTableIterator implements Iterator<Item<K, V>> {
        ...
        private Iterator<Item<K, V>> getNextIterator() {
            ...
            while(hashTableIndex < ExpChainingHashTable.this.hashTable.length &&
                  ExpChainingHashTable.this.hashTable[hashTableIndex] == null){
                hashTableIndex++;
            }
            if(hashTableIndex == ExpChainingHashTable.this.hashTable.length){
                throw new IndexOutOfBoundsException();
            }
            //invariant: hashTableIndex points to the index of the first dictionary in the hashtable
            return ExpChainingHashTable.this.hashTable[hashTableIndex].iterator();
        }

        @Override
        public boolean hasNext() {
            return itemsSeen < ExpChainingHashTable.this.size();
        }
